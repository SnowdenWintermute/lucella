...
export const requestPasswordResetEmail = (email) => async (dispatch) => {
  ...
  try {
    const res = await axios.post(`${apiUrl}/api/auth/request-password-reset`, body, config);
...
export const resetPassword =
  ({ password, password2, token, history }) =>
  async (dispatch) => {
    const config = {
      headers: {
        "Content-Type": "application/json",
    ...
    try {
      const res = await axios.post(`${apiUrl}/api/users/reset-password/${token}`, body, config);
      dispatch({
        type: LOGIN_SUCCESS,
...
export const deleteAccount =
  ...
  async (dispatch) => {
    ...
    try {
      const res = await axios.post(`${apiUrl}/api/users/delete-account`, body, config);
