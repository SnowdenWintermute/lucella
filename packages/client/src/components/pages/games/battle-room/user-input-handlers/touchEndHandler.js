...
export default ({ e, commonEventHandlerProps }) => {
  ...
  const adjustedOffsetY = (offsetY / canvasSize.height) * currentGameData.current.height;
  ...
  let type, props;
  if (touchLength > 500 || Math.abs(adjustedOffsetX - touchStartX) > 8 || Math.abs(adjustedOffsetY - touchStartY) > 8) {
