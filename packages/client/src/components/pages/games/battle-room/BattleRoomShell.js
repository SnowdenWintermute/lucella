...
const BattleRoomShell = (props) => {
  ...
  const inGameShellClass = gameStatus === "inProgress" || gameStatus === "ending" ? "game-shell-in-game" : "";

  return (
    <section className={gameStatus !== "inProgress" && gameStatus !== "ending" ? "page-frame" : ""}>
      <div className={`game-shell ${inGameShellClass}`}>
        {gameStatus !== "inProgress" && gameStatus !== "ending" ? <h1 className="game-page-title">Battle Room</h1> : ""}
