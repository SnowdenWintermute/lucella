...
export default function matchmakingQueueMenu() {
  ...
  it("correctly displays and provides functionality in the matchmaking queue menu", () => {
    ...
    cy.findByText(`${APP_TEXT.MATCHMAKING_QUEUE.NUM_PLAYERS_IN_QUEUE}1`).should("be.visible");
    cy.findByText(new RegExp(`${APP_TEXT.MATCHMAKING_QUEUE.ELO_DIFF_THRESHOLD}.*`)).should("be.visible");
    ...
    cy.findByText(`${APP_TEXT.MATCHMAKING_QUEUE.NUM_PLAYERS_IN_QUEUE}2`).should("be.visible");
