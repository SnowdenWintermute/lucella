...
export default function unauthedJoinLeaveAndDisconnectFromGameRoom() {
  it("unauthed user can see a list of games update correctly and join a game and be kicked from a game", () => {
    ...
    cy.task(TaskNames.socketEmit, { username: anonUsernameForCypressSocketList, event: SocketEventsFromClient.HOSTS_NEW_GAME, data: veryLongTestText });
    cy.get('[data-cy="refresh-button"]').click();
    cy.findByText(new RegExp(veryLongTestText, "i")).should("not.exist");
    cy.task(TaskNames.socketEmit, { username: anonUsernameForCypressSocketList, event: SocketEventsFromClient.HOSTS_NEW_GAME, data: shortTestText });
    cy.get('[data-cy="refresh-button"]').click();
    cy.findByText(new RegExp(shortTestText, "i")).should("exist");
    cy.task(TaskNames.socketEmit, { username: anonUsernameForCypressSocketList, event: SocketEventsFromClient.LEAVES_GAME, data: shortTestText });
    cy.get('[data-cy="refresh-button"]').click();
    ...
    cy.task(TaskNames.socketEmit, { username: anonUsernameForCypressSocketList, event: SocketEventsFromClient.HOSTS_NEW_GAME, data: shortTestText2 });
    cy.get('[data-cy="refresh-button"]').click();
