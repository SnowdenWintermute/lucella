...
export function processPlayerInput(input: UserInput, game: BattleRoomGame, deltaT: number, playerRole?: PlayerRole, clientPersistentGameCopy?: BattleRoomGame) {
  switch (input.type) {
    case UserInputs.CLIENT_TICK_NUMBER:
      updateOrbs(game, playerRole, clientPersistentGameCopy, input.number);
      break;
    case UserInputs.SELECT_ORBS:
      handleSelectOrbs(input, game);
      break;
    case UserInputs.ASSIGN_ORB_DESTINATIONS:
      handleAssignOrbDestinations(input, game, clientPersistentGameCopy, playerRole);
      break;
