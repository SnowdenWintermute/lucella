...
class Agent(nn.Module):
    def __init__(self, num_actions):
        ...
        self.network = nn.Sequential(
            self._layer_init(nn.Linear(30,60)),
            nn.ReLU(),
            self._layer_init(nn.Linear(60,60)),
            nn.ReLU(),
            nn.Flatten(),
            self._layer_init(nn.Linear(30,3)),
            nn.ReLU(),
        )

        self.actor = self._layer_init(nn.Linear(3, num_actions), std=0.01)
        self.critic = self._layer_init(nn.Linear(3, 1))
