...
export default async function accountActivationHandler(req: Request<object, object, { token: string }>, res: Response, next: NextFunction) {
  try {
    ...
    if (error.schema && error.detail) {
      ...
      if (error.code === "23505" && error.constraint === "users_email_key") errors.push(new CustomError(ErrorMessages.AUTH.EMAIL_IN_USE_OR_UNAVAILABLE, 403));
      if (error.code === "23505" && error.constraint === "users_name_key") errors.push(new CustomError(ErrorMessages.AUTH.NAME_IN_USE_OR_UNAVAILABLE, 403));
