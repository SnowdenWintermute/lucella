...
export default async function banIpAddressHandler(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!lucella.server) return next([new CustomError(ErrorMessages.SERVER_GENERIC, 500)]);
    if (!lucella.server.connectedUsers[name]) {
      ...
      return next([new CustomError(ErrorMessages.SERVER_GENERIC, 500)]);
    }
    ...
    if (!ipAddress) return next([new CustomError(ErrorMessages.ADMIN.NO_IP_TO_BAN, 404)]);
