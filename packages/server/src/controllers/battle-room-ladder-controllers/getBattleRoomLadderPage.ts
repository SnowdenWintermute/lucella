...
export default async function getBattleRoomLadderPage(req: Request, res: Response, next: NextFunction) {
  ...
  if (!inRedis) {
    ...
    return next([new CustomError(`${ERROR_MESSAGES.LADDER.NO_ENTRIES_FOUND}no ladder entries in redis`, 404)]);
  }

  const ids = inRedis.map((item) => parseInt(item.value, 10));
  if (!ids.length) {
    console.log("redis entries didn't contain ids");
    return next([new CustomError(`${ERROR_MESSAGES.LADDER.NO_ENTRIES_FOUND}redis entries didn't contain ids`, 404)]);
  }
  ...
  if (!scoreCardsWithUsernames) {
    ...
    return next([new CustomError(`${ERROR_MESSAGES.LADDER.NO_ENTRIES_FOUND}didn't find score cards in psql`, 404)]);
  }
