...
export default function (io: Server, serverState: ServerState, players: { host: RankedQueueUser; challenger: RankedQueueUser }) {
  ...
  handleReadyClick(io, io.sockets.sockets.get(players.host.socketId), serverState);
  handleReadyClick(io, io.sockets.sockets.get(players.challenger.socketId), serverState);
}
